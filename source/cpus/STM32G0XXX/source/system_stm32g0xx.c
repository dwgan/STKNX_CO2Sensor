...
VOID McuDly_DecUs(WORD32 dwCnt)
{
    __asm("subs    r0, #1\n"
          "bne     McuDly_DecUs\n"
          "bx      lr");
...
VOID McuUp_DelayUs(WORD32 dwUs)
{
    WORD32 dwCnt,dwSysCycNs;
    float  fTmp;
    
    fTmp       = 1.00/SYS_MCLK;
    dwSysCycNs = (WORD32)(fTmp*1000);
    dwCnt = ((dwUs*1000)/dwSysCycNs)/4;
    if(0 != dwCnt)
    {
        McuDly_DecUs(dwCnt);
    }
...
void PowerUp_Dly(void)
{
    McuUp_DelayUs(POWERUP_DLY_TIME);
