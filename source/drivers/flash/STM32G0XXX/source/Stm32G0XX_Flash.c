...
static WORD32 sg_dwFlashDelayCunt;
/* function type. */
...
#endif
{
    if(FTFA_CTRL & FLASH_CR_LOCK_Msk)
    {
        FTFA_KEY = FLASH_KEY1;
        FTFA_KEY = FLASH_KEY2;
    }
...
/**************************************************************************
* function name   : static VOID Stm32G0xxx_Flash_Option_Byte_Unlock(VOID)
* Description     : Unlock the option byte programming.
* Input parameter : no.
* Output parameter: no.
* Return          : no.
* Other description:
...
* 2016-08-03      V1.00                 Creation
**************************************************************************/
static VOID Stm32G0xxx_Flash_Option_Byte_Unlock(VOID)
{
    if(FTFA_CTRL & FLASH_CR_OPTLOCK_Msk)
    {
        FTFA_OPTKEY = FLASH_OPTKEY1;
        FTFA_OPTKEY = FLASH_OPTKEY2;
    }
...
/*************************************************************************
* Function name : void FlashDelay_DecUs(WORD32 dwCnt)
* Description : decrease function for Us delay.
* Input para. : dwCnt : counter for decreasing.
* Output para. : No
* Return : No
* Other description : it lives in RAM.
* Date       version     modify by      content
* ------------------------------------------------------------------------
* 2014-03-20   V1.00                       create
**************************************************************************/
__ramfunc VOID FlashDelay_DecUs(WORD32 dwCnt)
{
    __asm("subs    r0, #1\n"
          "bne     FlashDelay_DecUs\n"
          "bx      lr");
}
...
static WORD32 Stm32G0xxx_Flash_Init(T_SW_DEVICEDRIVER_PARA *ptPara)
{
    ...
    /* time delay counter. */
    fTmp                = 1.00/SYS_MCLK;
    dwSysCycNs          = (WORD32)(fTmp*1000);
    sg_dwFlashDelayCunt = ((FTFA_CMD_DELAY_STEP * 1000)/dwSysCycNs)/4;
...
#endif
{
    ...
    WORD16 wCunt;
    WORD32 dwError,dwEccErr;
    
    wCunt = 0;
    while(FTFA_ST & FLASH_SR_BSY)
    {
        /* busy,we wait for the end. */
        if(wCunt > FTFA_CMD_ERASE_TIME)
        {
            /* time is over. */
            ucRet = FLASH_ST_BUSY;
            break;
        ...
        {
            FlashDelay_DecUs(sg_dwFlashDelayCunt);
            wCunt++;
        }
    }
    /* clear the flag. */
...
/********************************************************************************************************************
* function name   : static WORD32 Stm32G0xxx_Flash_Read_Protect_Ctrl(T_SW_DEVICEDRIVER_PARA *ptPara)
* Description     : Flash read protect control.
* Input parameter : T_SW_DEVICEDRIVER_PARA *ptPara 
*                                           BYTE   ucPara[2]
*                                                  EN_READ_PROTECT  eanble read protection
*                                                  DIS_READ_PROTECT disable read protection
*                              
*                                                  ucPara[3]        user data0
*                                                  ucPara[4]        user data1
* Output parameter: no.
...
* 2016-08-03      V1.00                 Creation
*********************************************************************************************************************/
static WORD32 Stm32G0xxx_Flash_Read_Protect_Ctrl(T_SW_DEVICEDRIVER_PARA *ptPara)
{
    BYTE   ucStatus;
    WORD16 wCunt;
    
    if((EN_READ_PROTECT != ptPara->ucPara[2]) && (DIS_READ_PROTECT != ptPara->ucPara[2]))
    {
        return SW_ERR;
    }
    /* Check command */
    if(EN_READ_PROTECT == ptPara->ucPara[2])
    {
        if(FLASH_READ_PROTECT_LEVEL_0 != (FTFA_OPTBYTE & FLASH_OPTR_RDP))
        {
            /* Read protection has been enabled, do not operate */
            return SW_OK;
        }
    }
    /* unlock the flash. */
    ...
    /* Unlock option byte */
    Stm32G0xxx_Flash_Option_Byte_Unlock();
    
    /* Get the opration time */
    wCunt = 0;
    /* Wait option byte write enable */
    while((FTFA_CTRL & FLASH_CR_OPTLOCK_Msk) != 0)
    {
        /* busy,we wait for the end. */
        if(wCunt > FTFA_CMD_ERASE_TIME)
        {
    ...
    if(EN_READ_PROTECT == ptPara->ucPara[2])
    {
        /* Write value to option byte address */
        FTFA_OPTBYTE = (FTFA_OPTBYTE & (~FLASH_OPTR_RDP)) | FLASH_READ_PROTECT_LEVEL_1;
    }
    else
    {
        /* Write value to option byte address */
        FTFA_OPTBYTE = (FTFA_OPTBYTE & (~FLASH_OPTR_RDP)) | FLASH_READ_PROTECT_LEVEL_0;
    }
    ...
    /* Clear option byte programming flag */
    FTFA_CTRL |= FLASH_CR_OPTSTRT;
    /* check the status of flash. */
    ucStatus = Stm32G0xxx_Flash_WaitForOp();
    if(FLASH_ST_BUSY == ucStatus)
    {
...
/*******************************************************************************************************************
* function name   : static WORD32 Stm32F10xx_Flash_Ctrl(T_SW_DEVICEDRIVER_PARA *ptPara)
* Description     : Initialize the flash.
* Input parameter : T_SW_DEVICEDRIVER_PARA *ptPara 
*                                           BYTE   ucPara[2]
*                                                  FLASH_MASS_ERASE  Flash mass erase
*                                                  EN_READ_PROTECT   enable read protection.
*                                                  DIS_READ_PROTECT  disable read protection.
*                              
*                                                  ucPara[3]         user data0
*                                                  ucPara[4]         user data1
...
* 2016-08-03      V1.00                 Creation
*******************************************************************************************************************/
static WORD32 Stm32G0xxx_Flash_Ctrl(T_SW_DEVICEDRIVER_PARA *ptPara)
{
    BYTE ucStatus;
    if((NULL == ptPara) || (SW_PERIPHERAL_FLASH != ptPara->ucPara[0]))
    {
        return SW_ERR;
    }
    if((FLASH_MASS_ERASE != ptPara->ucPara[2]) && (EN_READ_PROTECT != ptPara->ucPara[2]) && (DIS_READ_PROTECT != ptPara->ucPara[2]))
    {
        return SW_ERR;
    }
    
    /* Check command */
    if(FLASH_MASS_ERASE == ptPara->ucPara[2])
    {
        /* unlock the flash. */
        Stm32G0xxx_Flash_Unlock();
        /* check the status of flash. */
    ...
    {
        Stm32G0xxx_Flash_Read_Protect_Ctrl(ptPara);
    }
...
* Input parameter : BYTE ucLogNo : device's logical no.
                    ...
                    VOID *ptPara : handler of time delay operation.
* Output parameter: no.
* Return          : no.
* Other description:
* Modify date     Ver.     Modifier      Action
...
VOID Stm32G0xxx_Flash_Reg(BYTE ucLogNo,BYTE ucDevNo,VOID *ptPara)
{
    ...
    sg_tStm32F10xxFlashOp.pfDrvCtrl  = Stm32G0xxx_Flash_Ctrl;
