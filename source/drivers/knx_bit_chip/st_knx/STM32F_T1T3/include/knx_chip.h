...
#define KNXCHIP_TO_MCU                 GET_GPIO_PORT_DATA(A,8)                     /* getting bit from knx chip. Attention: this pin is fixed to A8, or the driver has error.*/
#define KNXCHIP_TO_MCU_PA8             GET_GPIO_PORT_DATA(A,8)                     /* getting bit from knx chip. Attention: this pin is fixed to A8, or the driver has error.*/
#define KNXCHIP_TO_MCU_PA9             GET_GPIO_PORT_DATA(A,9)                     /* getting bit from knx chip. Attention: this pin is fixed to A9, or the driver has error.*/
#define MCU_TO_KNXCHIP                 GET_GPIO_PORT_DATA(A,7)                     /* getting bit from the output ping. */
#define RX_LINE_IS_ACTIVE              (KNXCHIP_TO_MCU != 0)
#define RX_LINE_IS_ACTIVE_PA8          (KNXCHIP_TO_MCU_PA8 != 0)
#define RX_LINE_IS_ACTIVE_PA9          (KNXCHIP_TO_MCU_PA9 != 0)
#define TX_LINE_IS_ACTIVE              (MCU_TO_KNXCHIP != 0)

#define SET_KNXCHIP_HIGH               SET_GPIO_PORT(A,8)
...
#define BYTE_RX_IS_ONGOING             ((TIM1->DIER & TIM_DIER_CC3IE) != 0)
#define BYTE_RXCTRL_TMINT_EN             TIM1->DIER |=  TIM_DIER_CC3IE
#define BYTE_RXCTRL_TMINT_DIS            TIM1->DIER &= ~TIM_DIER_CC3IE
#define BIT_RISE_EDGE_INT_PENDING      ((TIM1->SR   &  TIM_SR_CC1IF)  != 0)
#define BIT_FALL_EDGE_INT_PENDING      ((TIM1->SR   &  TIM_SR_CC2IF)  != 0)
#define BIT_RISE_INT_DISABBLE            TIM1->DIER &= ~TIM_DIER_CC1IE
#define BIT_FALL_INT_DISABBLE            TIM1->DIER &= ~TIM_DIER_CC2IE
#define BIT_RISE_INT_ENABLE              TIM1->DIER |= TIM_DIER_CC1IE
#define BIT_FALL_INT_ENABLE              TIM1->DIER |= TIM_DIER_CC2IE
#define BIT_RX_CAP_INT_CLR               TIM1->SR   &= ~(TIM_SR_CC1F | TIM_SR_CC2F)
#define BIT_RX_RISE_CAP_INT_CLR          TIM1->SR   &= ~TIM_SR_CC1F
#define BIT_RX_FALL_CAP_INT_CLR          TIM1->SR   &= ~TIM_SR_CC2F

#define BYTE_RXCTRL_TIMER_SET(wStartBitTime)   \
            {\
                TIM1->CCR3  = ((WORD16)(wStartBitTime + BYTE_TIM));\
                TIM1->SR   &= (~TIM_SR_CC3F);\
...
#define BIT_RISING_EDGE_CAP_START \
            {\
                TIM1->SR   &= ~(TIM_SR_CC1F | TIM_SR_CC2F);  \
...
#define BIT_ALL_EDGE_CAP_STOP \
            {\
                ...
                TIM1->DIER &= ~(TIM_DIER_CC2IE | TIM_DIER_CC1IE);\
                TIM1->SR   &= ~(TIM_SR_CC1F    | TIM_SR_CC2F);   \
            }
...
#define SYNC_KNXPH_TIMER           sg_wTimCnt = TIM1->CNT; TIM3->CNT = sg_wTimCnt
#define IS_OUTPUT_WAITTO_ACTIVE    ((TIM3->CCMR1 & TIM_CCMR1_OC2M_0) != 0)

#define ENABLE_RX_TIMER            TIM1->CR1 |= TIM_CR1_CEN
#define RX_TIME_EN_CHECK           (0==(TIM1->CR1 & TIM_CR1_CEN))
