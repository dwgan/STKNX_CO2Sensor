...
*
* File Name        : Stm32G0xxx_uart.c
*
* File Description : driver for stm32f0xxx cpu's uart.
*
...
WORD32 Uartx_Ctrl(T_SW_DEVICEDRIVER_PARA *ptPara)
{
    ...
    switch (ptPara->ucPara[UART_OPERATION_CMD])                                
    {
        ...
        {
            UARTX_CR1(ptUartNo) &= ~(USART_CR1_TXEIE_TXFNFIE);    /* Disable Tx interrupt            */
            break;
        }

        case SW_COMM_RXINT_DISEN :                        /* Command to disable Rx interrupt */
        {
            UARTX_CR1(ptUartNo) &= ~(USART_CR1_RXNEIE_RXFNEIE);   /* Disable Rx interrupt            */
            break;
        }

        case SW_COMM_TXINT_EN :                           /* Command to enable Tx interrupt  */              
        {
            UARTX_CR1(ptUartNo) |= USART_CR1_TXEIE_TXFNFIE;       /* Enable Tx interrupt             */                          
            break;
        }

        case SW_COMM_RXINT_EN :                           /* Command to enable Rx interrupt  */               
        {
            UARTX_CR1(ptUartNo) |= USART_CR1_RXNEIE_RXFNEIE;      /* Enable Rx interrupt             */                            
            break;
        }
        ...
        case SW_COMM_DEV_CONFIG :                         /* Command to configure uart       */
        {
            ...
            switch (ptPara->ucPara[UART_BIT_MODE])        /* Check the bit mode              */
            {
                ...
                {
                    UARTX_CR1(ptUartNo) |= USART_CR1_M0;   /* Set 9-bit mode                  */
                    break;
                }
...
WORD32 Uartx_Read(T_DATA_OPERARTION *ptData)
{
    ...
    {
        UARTX_ICR(ptUartNo) = (WORD32)0x000f;                 /* Clear error status */
...
WORD32 Uartx_Write(T_DATA_OPERARTION *ptData)
{
    ...
    UARTX_TDR(ptUartNo) = *(ptData->pucBuffer);
...
static void Uartx_isr(BYTE ucDevNo)
{
    ...
    if(UARTX_SR(ptUartNo) & USART_SR_ERR_MASK)
    {
        ...
        UARTX_ICR(ptUartNo) = (WORD32)0x000f;
    ...
    else if(UARTX_CR1(ptUartNo) & USART_CR1_TXEIE_TXFNFIE)
    {
        /* Disable Tx interrupt */
        UARTX_CR1(ptUartNo) &= ~(USART_CR1_TXEIE_TXFNFIE);
